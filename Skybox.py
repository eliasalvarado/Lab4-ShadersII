...
class Skybox(object):
	...
	def loadEnvironmentMap(self, textureFile):
		faces = ["negx.jpg", "negy.jpg", "negz.jpg", "posx.jpg", "posy.jpg", "posz.jpg"]
		...
		for i in range(6):
			...
			imageData = pygame.image.tostring(image, "RGB", True)
			glTexImage2D(
	...
	def render(self, viewMatrix, projectionMatrix):
		...
		glUniformMatrix4fv(glGetUniformLocation(self.activeShader, "viewMatrix"), 1, GL_FALSE, glm.value_ptr(viewMatrix))
		...
		glUniform1i(glGetUniformLocation(self.activeShader, "skybox"), 1)
